...
#define PI 3.1415926535
      ...
      sdf::ElementPtr sdf) 
  {
    ...
    pose2d_pub = nh_->advertise<geometry_msgs::Pose2D>("odom_pose2d", 50);
  ...
  void GazeboRosModelVelocity::UpdateChild()
  {
    ...
    if (dt > ros::Duration(1.0 / update_rate_)){
      ...
      //double y_vel_cmd = cmd.linear.x * cos(yaw+PI/2.0) + cmd.linear.y * sin(yaw+PI/2.0);
      double y_vel_cmd = cmd.linear.x * sin(yaw) + cmd.linear.y * cos(yaw);

      parent_->SetLinearVel(math::Vector3(x_vel_cmd*3.0, y_vel_cmd*3.0, 0.0));
      parent_->SetAngularVel(math::Vector3(0.0, 0.0, cmd.angular.z*2.2));

      cmd.linear.x=x_vel_cmd;
      cmd.linear.y=y_vel_cmd;
      cmd.angular.x=yaw;
  ...
  void GazeboRosModelVelocity::publishOdometry(double step_time)
  {
    ...
    // tf::poseTFToMsg(odom_transform_, odom_.pose.pose);
      ...
      math::Pose getpose = parent_->GetRelativePose();
      double yaw = getpose.rot.GetYaw();
      geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(yaw);

        geometry_msgs::Pose2D pose2d;
        pose2d.x = getpose.pos[0];
        pose2d.y = getpose.pos[1];
        pose2d.theta = yaw;
        pose2d_pub.publish(pose2d);


      odom_.pose.pose.position.x = getpose.pos[0];
      odom_.pose.pose.position.y = getpose.pos[1];
      odom_.pose.pose.position.z = getpose.pos[2];
    ...
    odom_trans.transform.translation.x = getpose.pos[0];
    odom_trans.transform.translation.y = getpose.pos[1];
